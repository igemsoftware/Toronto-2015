...
angular.module('ConsortiaFlux').controller('CompareModelsModal', CompareModelsModal);

function CompareModelsModal($http, UrlProvider, close) {
    this._http = $http;
    this.UP = UrlProvider;
...
CompareModelsModal.inject(['$http', 'UrlProvider']);

CompareModelsModal.prototype.load = function(choosable) {
...
CompareModelsModal.prototype.go = function() {
...
CompareModelsModal.prototype.disabled = function(model) {
...
CompareModelsModal.prototype.checkBoxHandler = function(model) {
    if ($scope.chosenModels.indexOf(model) === -1) {
...
CompareModelsModal.send = function() {

    this.sended.models = $scope.chosenModels;
    ...
    var send = {
        id1: this.sended.models[0],
        id2: this.sended.models[1],
        type: this.sended.type
    }
    ...
    this._http.post(requestUrl, send).then(this.receiver.bind(this), this.errorCatch.bind(this));
}

CompareModelsModal.prototype.close = function() {
    this.display = false;
    close(null);
};
